...
import { Store, Package, PackageOpen, ArrowRight } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useGame } from '../../context/GameContext';
import { AssetPoeiraVital, AssetMusgo, AssetEssenciaSombria, AssetNucleoGolem, AssetReliquia } from './GameAssets';
...
export default function Loja() {
  ...
  const sellItem = (itemKey: string, amount: number, price: number) => {
    if (inventory[itemKey] >= amount) {
      ...
      alert("Recursos insuficientes para transacionar!");
    }
  };
  ...
  const buyItem = (itemKey: string, amount: number, price: number) => {
    if (gold >= price) {
      ...
      alert("Poeira Vital insuficiente!");
    }
  };
  ...
  const openChest = (type: 'comum' | 'epico') => {
    ...
    setTimeout(() => {
        ...
        setMyCards((prev: any) => [...prev, { ...selectedCard, id: `c_${Date.now()}`, level: 1 }]);
    }, 2000);
  };
    ...
    <div className="p-4 flex flex-col items-center h-full overflow-y-auto no-scrollbar pb-32">
      <div className="w-full max-w-5xl flex flex-col gap-6">
        ...
        <div className="bg-[#042f2e] border-4 border-[#022c22] p-8 rounded-[2.5rem] text-center w-full shadow-xl relative overflow-hidden flex flex-col items-center">
            ...
            <div className="bg-teal-800 p-4 rounded-[2rem] border-4 border-teal-600 shadow-md mb-4 relative z-10">
                <Store size={48} className="text-teal-200" strokeWidth={3} />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#042f2e] border-4 border-[#022c22] rounded-[2.5rem] p-6 shadow-xl flex flex-col">
                <h3 className="text-base font-black uppercase tracking-widest text-teal-500 mb-6 flex items-center gap-2">
                    <Package size={20} strokeWidth={3}/> Relicários
                </h3>
                ...
                <div className="flex flex-col gap-4">
                    <div className="bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-5 flex items-center justify-between group shadow-lg">
                        <div className="flex items-center gap-4">
                            <Package className="w-12 h-12 text-teal-500 drop-shadow-md" strokeWidth={2}/>
                            <div>
                                <h4 className="text-sm font-black text-teal-100 uppercase">Baú Rúnico</h4>
                                <p className="text-[10px] text-teal-600 font-bold uppercase mt-1 bg-[#042f2e] px-2 py-0.5 rounded-full border-2 border-[#022c22] w-max">Revela: Comum a Épica</p>
                            </div>
                        </div>
                        <button onClick={() => openChest('comum')} className="bg-teal-600 hover:bg-teal-500 text-white px-4 py-3 rounded-2xl text-xs font-black flex items-center gap-2 transition-all border-4 border-teal-800 border-b-8 active:border-b-4 active:translate-y-1 shadow-sm">
                            500 <AssetPoeiraVital className="w-4 h-4" />
                        </button>
                    </div>
                    ...
                    <div className="bg-[#022c22] border-4 border-purple-900/50 rounded-[2rem] p-5 flex items-center justify-between group shadow-[0_0_15px_rgba(168,85,247,0.1)]">
                        <div className="flex items-center gap-4">
                            <PackageOpen className="w-12 h-12 text-purple-500 drop-shadow-[0_0_10px_rgba(168,85,247,0.5)]" strokeWidth={2}/>
                            <div>
                                <h4 className="text-sm font-black text-purple-300 uppercase">Relicário Arcano</h4>
                                <p className="text-[10px] text-purple-400 font-bold uppercase mt-1 bg-[#042f2e] px-2 py-0.5 rounded-full border-2 border-[#022c22] w-max">Garante: Rara ou Superior</p>
                            </div>
                        </div>
                        <button onClick={() => openChest('epico')} className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-3 rounded-2xl text-xs font-black flex items-center gap-2 transition-all border-4 border-purple-800 border-b-8 active:border-b-4 active:translate-y-1 shadow-sm">
                            2.500 <AssetPoeiraVital className="w-4 h-4" />
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#042f2e] border-4 border-[#022c22] rounded-[2.5rem] p-6 shadow-xl flex flex-col">
                <h3 className="text-base font-black uppercase tracking-widest text-teal-500 mb-6 flex items-center gap-2">
                    <ArrowRight size={20} strokeWidth={3}/> Comércio de Recursos
                </h3>
            ...
            <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
                {chestStage === 'shaking' ? (
                    <motion.div animate={{ x: [-10, 10, -10, 10, 0], y: [-5, 5, -5, 5, 0], scale: [1, 1.1, 1] }} transition={{ duration: 0.5, repeat: 3 }} className="text-[8rem] drop-shadow-[0_0_40px_rgba(255,255,255,0.4)]">
                        {openingChest.type === 'comum' ? <Package className="w-32 h-32 text-teal-500" /> : <PackageOpen className="w-32 h-32 text-purple-500" />}
                    </motion.div>
                ) : (
                    <motion.div initial={{ scale: 0.5, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="flex flex-col items-center">
                        <div className="text-5xl text-yellow-400 mb-6 animate-pulse">✨</div>
                        <div className={`w-56 h-72 border-4 border-b-[12px] ${getRarityColor(openingChest.card.rarity)} rounded-[2rem] p-4 flex flex-col items-center justify-center shadow-[0_0_50px_rgba(255,255,255,0.2)] relative`}>
                            ...
                            <div className="w-24 h-24 bg-black/20 rounded-2xl mb-4 flex items-center justify-center overflow-hidden border-4 border-black/20 shadow-inner">
                                <img src={getAvatarUrl(openingChest.card.name)} alt={openingChest.card.name} className="w-20 h-20 drop-shadow-md opacity-90" />
                            </div>

                            <span className="text-xs font-black uppercase mb-2 bg-black/20 text-white/90 px-3 py-1 rounded-full">{openingChest.card.rarity}</span>
                            <h3 className="text-lg font-black mb-4 text-center leading-tight px-2">{openingChest.card.name}</h3>
                            
                            <div className="flex gap-3 text-xs font-black w-full justify-center">
                                <span className="text-rose-200 bg-rose-950/50 px-3 py-1.5 rounded-xl shadow-sm border-2 border-rose-900/50">ATK {openingChest.card.atk}</span>
                            </div>
                        </div>
...
const TradeRow = ({ label, icon, costText, rewardText, canAfford, action, isBuy = false }: any) => (
    <div className="bg-[#022c22] border-4 border-[#064e3b] rounded-2xl p-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-4">
            <div className="flex items-center justify-center bg-[#042f2e] p-2 rounded-xl border-2 border-[#0f766e] shadow-sm">{icon}</div>
            <div className="flex flex-col">
                <span className="text-sm font-black text-teal-100 uppercase">{label}</span>
                <span className="text-[10px] font-black text-teal-600 uppercase mt-1">{costText} ➔ {rewardText}</span>
            </div>
        </div>
        <button onClick={action} disabled={!canAfford} className={`${isBuy ? 'bg-purple-600 border-purple-800 text-white' : 'bg-teal-600 border-teal-800 text-white'} border-4 border-b-8 active:border-b-4 active:translate-y-1 px-4 py-3 rounded-2xl text-xs font-black uppercase transition-all disabled:opacity-30 disabled:grayscale shadow-sm`}>
