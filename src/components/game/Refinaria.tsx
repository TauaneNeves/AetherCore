...
import { AssetEssenciaSombria, AssetFragmentoRunico, AssetPoeiraVital, AssetNucleoGolem, AssetCristalAether, AssetFire, AssetSparkles, AssetArrowRight } from './GameAssets';
...
export default function Refinaria() {
  ...
  const processForge = (type: 'ferro' | 'ouro') => {
      const oreCost = 5; const fuelCost = type === 'ouro' ? 2 : 1;
  ...
  const craftRobot = (tier: number) => {
      const pecasReq = tier === 1 ? 10 : 25; const ferroReq = tier === 1 ? 5 : 15;
      if (inventory.pecas_robo >= pecasReq && (refinedBars.ferro || 0) >= ferroReq) {
          setIsCrafting(true); setCraftProgress(0);
          let currentProgress = 0;
          const t = setInterval(() => {
              currentProgress += 5; setCraftProgress(currentProgress);
              if (currentProgress >= 100) {
                  clearInterval(t); setIsCrafting(false);
            ...
            {activeTab === 'transmutacao' && (
                <motion.div key="transmutacao" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-6">
                    <div className="bg-[#042f2e] border-4 border-[#022c22] rounded-[2.5rem] p-8 shadow-xl flex flex-col items-center relative overflow-hidden">
                        <AssetFire className={`w-16 h-16 mb-6 ${isRefining ? 'text-purple-400 animate-pulse drop-shadow-[0_0_15px_rgba(168,85,247,0.8)]' : 'text-[#0f766e]'}`} />
                        ...
                        <div className="flex flex-col md:flex-row w-full gap-4">
                            <div className="flex-1 bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-6 flex flex-col items-center justify-between shadow-lg">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <span className="text-teal-100 font-black text-xl">5</span><span className="text-teal-700 font-black text-xl">+</span>
                                    ...
                                    <AssetArrowRight className="w-6 h-6 text-teal-500 mx-2"/>
            ...
            {activeTab === 'animacao' && (
                <motion.div key="animacao" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-6">
                    <div className="bg-[#042f2e] border-4 border-[#022c22] rounded-[2.5rem] p-8 shadow-xl flex flex-col items-center">
                        <AssetSparkles className={`w-16 h-16 mb-6 ${isCrafting ? 'text-teal-300 animate-pulse drop-shadow-[0_0_15px_rgba(45,212,191,0.8)]' : 'text-[#0f766e]'}`} />
                        ...
                        {isCrafting && (<div className="w-full max-w-md h-4 bg-[#022c22] mb-8 rounded-full overflow-hidden border-4 border-[#064e3b]"><div className="h-full bg-teal-400" style={{width: `${craftProgress}%`}}/></div>)}

                        <div className="flex flex-col md:flex-row w-full gap-4">
                            <div className="flex-1 bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-6 flex flex-col shadow-lg">
                                <div className="flex justify-between items-start mb-6"><div><h3 className="text-base font-black text-teal-100 uppercase">Golem Menor</h3><p className="text-xs text-teal-500 uppercase font-bold mt-1">Taxa: 1 / Ciclo</p></div><div className="w-16 h-16 bg-black/20 rounded-2xl border-2 border-black/10 opacity-80 flex items-center justify-center overflow-hidden"><AssetNucleoGolem className="w-10 h-10"/></div></div>
                                <div className="flex gap-4 text-base font-black mb-6 bg-[#042f2e] p-4 rounded-2xl border-2 border-[#0f766e] justify-center items-center"><span className="text-teal-100 flex items-center gap-2"><AssetNucleoGolem className="w-5 h-5" /> 10</span><span className="text-teal-100 flex items-center gap-2"><AssetFragmentoRunico className="w-5 h-5 opacity-70" /> 5</span></div>
                            ...
                            <div className="flex-1 bg-[#022c22] border-4 border-purple-900/50 rounded-[2rem] p-6 flex flex-col shadow-[0_0_20px_rgba(168,85,247,0.1)] relative overflow-hidden">
                                <div className="flex justify-between items-start mb-6 relative z-10"><div><h3 className="text-base font-black text-purple-300 uppercase">Golem Ancestral</h3><p className="text-xs text-purple-500 uppercase font-bold mt-1">Taxa: 2 / Ciclo</p></div><div className="w-16 h-16 bg-black/20 rounded-2xl border-2 border-black/10 opacity-80 flex items-center justify-center overflow-hidden"><AssetNucleoGolem className="w-12 h-12 text-purple-500"/></div></div>
                                <div className="flex gap-4 text-base font-black mb-6 bg-[#042f2e] p-4 rounded-2xl border-2 border-purple-900/50 justify-center items-center relative z-10"><span className="text-teal-100 flex items-center gap-2"><AssetNucleoGolem className="w-5 h-5" /> 25</span><span className="text-teal-100 flex items-center gap-2"><AssetFragmentoRunico className="w-5 h-5 opacity-70" /> 15</span></div>
