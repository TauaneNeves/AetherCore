...
import { 
    AssetFire, AssetWater, AssetEarthElem, AssetAir, AssetPoeiraVital, AssetFragmentoRunico, 
    AssetSkull, AssetClock, AssetReliquia, AssetSparkles, AssetTarget, AssetRuneStone,
...
const getElementIcon = (element: string) => {
    switch(element) { case 'Fogo': return <AssetFire className="w-4 h-4" />; case 'Água': return <AssetWater className="w-4 h-4" />; case 'Terra': return <AssetEarthElem className="w-4 h-4" />; case 'Ar': return <AssetAir className="w-4 h-4" />; default: return null; }
};

export default function BaseScreen() {
  const { activeDeck, gold, setGold, refinedBars, setRefinedBars, buildings, setBuildings, inventory, setInventory, research, setResearch, wave, setWave, soulShards, ascendCore, offlineReport, setOfflineReport } = useGame();
  ...
  const coreCostGold = buildings.core.level * 1000; const coreCostFerro = buildings.core.level * 2;
  const towerCostGold = (buildings.tower.level + 1) * 800; const towerCostFerro = (buildings.tower.level + 1) * 1;
  const shieldCostGold = (buildings.shield.level + 1) * 1500; const shieldCostOuro = (buildings.shield.level + 1) * 1;
  const radarCost = (research.radarCooldown + 1) * 2; const minerCost = (research.minerBonus + 1) * 3; const coreBonusCost = (research.coreBonus + 1) * 2;
  ...
  const getRarityColor = (rarity: string) => {
    switch(rarity) { case 'Comum': return 'bg-[#0f766e] border-[#042f2e] text-teal-100'; case 'Rara': return 'bg-[#0ea5e9] border-[#075985] text-sky-100'; case 'Épica': return 'bg-[#a855f7] border-[#581c87] text-purple-100'; case 'Lendária': return 'bg-[#f59e0b] border-[#92400e] text-amber-100'; default: return 'bg-slate-400 border-slate-600 text-slate-900'; }
  }
  ...
  useEffect(() => {
      if (combatState.status === 'idle') { const t = setInterval(() => { setCardCharges(prev => prev.map(c => Math.min(100, c + 5))); }, 500); return () => clearInterval(t); }
  }, [combatState.status]);

  const upgradeCore = () => {
    const ferroRefinado = refinedBars.ferro || 0;
    if (gold >= coreCostGold && ferroRefinado >= coreCostFerro) { setGold((g: number) => g - coreCostGold); setRefinedBars((prev: any) => ({ ...prev, ferro: prev.ferro - coreCostFerro })); setBuildings((prev: any) => ({ ...prev, core: { level: prev.core.level + 1, maxHp: prev.core.maxHp + 50, hp: prev.core.hp + 50 }}));
    } else alert("Recursos insuficientes!");
  };

  const upgradeTower = () => {
    const ferroRefinado = refinedBars.ferro || 0;
    if (gold >= towerCostGold && ferroRefinado >= towerCostFerro) { if (buildings.tower.level >= buildings.core.level) return alert("Fortaleça o Núcleo Vital primeiro!"); setGold((g: number) => g - towerCostGold); setRefinedBars((prev: any) => ({ ...prev, ferro: prev.ferro - towerCostFerro })); setBuildings((prev: any) => ({ ...prev, tower: { level: prev.tower.level + 1, atk: prev.tower.atk + 15 }}));
    } else alert("Recursos insuficientes!");
  };

  const upgradeShield = () => {
    const ouroRefinado = refinedBars.ouro || 0;
    if (gold >= shieldCostGold && ouroRefinado >= shieldCostOuro) { if (buildings.shield.level >= buildings.core.level) return alert("Fortaleça o Núcleo Vital primeiro!"); setGold((g: number) => g - shieldCostGold); setRefinedBars((prev: any) => ({ ...prev, ouro: prev.ouro - shieldCostOuro })); setBuildings((prev: any) => ({ ...prev, shield: { level: prev.shield.level + 1, max: prev.shield.max + 100, current: prev.shield.current + 100 }}));
    } else alert("Recursos insuficientes!");
  };

  const researchTech = (type: 'radar' | 'miner' | 'core') => {
      if (type === 'radar' && inventory.item_antigo >= radarCost) { setInventory((prev:any) => ({...prev, item_antigo: prev.item_antigo - radarCost})); setResearch((prev:any) => ({...prev, radarCooldown: prev.radarCooldown + 1})); } else if (type === 'miner' && inventory.item_antigo >= minerCost) { setInventory((prev:any) => ({...prev, item_antigo: prev.item_antigo - minerCost})); setResearch((prev:any) => ({...prev, minerBonus: prev.minerBonus + 1})); } else if (type === 'core' && inventory.item_antigo >= coreBonusCost) { setInventory((prev:any) => ({...prev, item_antigo: prev.item_antigo - coreBonusCost})); setResearch((prev:any) => ({...prev, coreBonus: prev.coreBonus + 1})); setBuildings((prev: any) => ({ ...prev, core: { ...prev.core, maxHp: prev.core.maxHp + 100, hp: prev.core.hp + 100 } })); } else { alert("Relíquias insuficientes!"); }
  };
  ...
  const simulateInvasion = () => {
    ...
    const newEnemy = { hp: enemyPower, maxHp: enemyPower, atk: enemyPower, seed: `Void_${Math.random()}`, element: enemyElement, isBoss };
    ...
    activeDeck.forEach((card: any) => {
        let advMult = 1.0;
        if (ELEMENT_ADVANTAGE[card.element] === enemyElement) advMult = 1.5; else if (ELEMENT_ADVANTAGE[enemyElement] === card.element) advMult = 0.8; 
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                else { setCombatState(prev => ({ ...prev, status: 'enemy_attack' })); setTimeout(() => finalizeBattle('defeat', hpAfterTower), 500); }
            }, 1200);
        }, 1500);
    }, 1500);
  };
  ...
  const finalizeBattle = (result: 'victory' | 'defeat', damageDealtToBase: number = 0) => {
    ...
    if (result === 'victory') {
        ...
        setWave(w => w + 1);
        if (isBoss) setInventory((prev:any) => ({...prev, item_antigo: (prev.item_antigo || 0) + 1}));
    } else {
        let newShield = buildings.shield.current;
        let newHp = buildings.core.hp;
        if (newShield > 0) { setShieldHit(true); if (damageDealtToBase >= newShield) { const remainder = damageDealtToBase - newShield; newShield = 0; newHp -= remainder; setCoreHit(true); } else { newShield -= damageDealtToBase; } } else { newHp -= damageDealtToBase; setCoreHit(true); }
    ...
    setTimeout(() => { 
        setCombatState({ status: 'idle', enemy: null, feedback: '' }); setShieldHit(false); setCoreHit(false); 
        if (isGameOver) { setBuildings((prev: any) => ({ ...prev, core: { ...prev.core, hp: prev.core.maxHp } })); alert("SISTEMA CRÍTICO! O seu Núcleo foi destruído. Suas defesas falharam na Onda " + wave + "."); }
    }, 3000);
  };

  const formatTime = (seconds: number) => { const h = Math.floor(seconds / 3600); const m = Math.floor((seconds % 3600) / 60); if (h > 0) return `${h}h ${m}m`; return `${m}m`; };
          ...
          {offlineReport && (
              <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                  <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#042f2e] border-4 border-[#022c22] rounded-[3rem] p-8 max-w-sm w-full flex flex-col items-center shadow-2xl relative overflow-hidden">
                      ...
                      <AssetClock className="w-12 h-12 text-teal-400 mb-4 animate-pulse relative z-10" />
            ...
            {activeTab === 'comando' && (
                <motion.div key="comando" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-6">
                    ...
                    <div className="relative h-[450px] w-full bg-[#042f2e] border-4 border-[#022c22] border-b-[12px] rounded-[2.5rem] overflow-hidden shadow-xl" style={{ perspective: '1200px' }}>
                        <div className="absolute top-4 left-4 z-50 bg-[#022c22] border-2 border-[#064e3b] px-4 py-2 rounded-2xl flex flex-col items-center shadow-lg"><span className="text-[9px] text-teal-600 font-black uppercase">Onda Atual</span><span className="text-xl text-white font-black">{wave}</span></div>
                        {wave > 5 && combatState.status === 'idle' && ( <button onClick={ascendCore} className="absolute top-4 right-4 z-50 bg-fuchsia-600 text-white border-2 border-fuchsia-400 px-4 py-2 rounded-2xl font-black text-[10px] uppercase shadow-[0_0_15px_rgba(192,38,211,0.6)] animate-pulse flex items-center gap-1 hover:scale-105 transition-transform">Sobrecarga</button> )}
                        ...
                        {combatState.feedback && ( <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute top-32 left-1/2 -translate-x-1/2 z-50 bg-purple-600 text-white px-4 py-1 rounded-full border-2 border-purple-400 font-black text-xs shadow-[0_0_20px_rgba(168,85,247,0.8)]">{combatState.feedback}</motion.div> )}
                        ...
                        <AnimatePresence>
                            {combatState.enemy && (
                                <motion.div initial={{ y: -100, opacity: 0, scale: 0.5 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: -100, opacity: 0, scale: 0.5 }} className="absolute top-[8%] left-1/2 -translate-x-1/2 z-50 flex flex-col items-center">
                                    ...
                                    {/* INIMIGO DA CORRUPÇÃO EM SVG! */}
                                    <div className={`bg-indigo-900 border-4 border-indigo-700 border-b-8 p-1 rounded-[2rem] shadow-xl relative z-10 animate-bounce overflow-hidden ${combatState.enemy.isBoss ? 'scale-125 border-rose-600 bg-rose-950 shadow-[0_0_30px_rgba(225,29,72,0.6)]' : ''}`}>
                                        <div className="absolute -top-3 -right-3 bg-[#022c22] border-2 border-[#0f766e] rounded-full w-8 h-8 flex items-center justify-center text-sm shadow-md z-20">
                                            {getElementIcon(combatState.enemy.element)}
                                        </div>
                                        <AssetEnemyCorruption className="w-16 h-16 p-2" isBoss={combatState.enemy.isBoss} />
                                    </div>
                        ...
                        <div className="absolute bottom-[12%] left-1/2 -translate-x-1/2 z-40 flex gap-4">
                            {activeDeck.map((card: any, idx: number) => {
                                ...
                                <motion.div key={card.id} onClick={() => isCombat && triggerUltimate(idx)} animate={combatState.status === 'cards_attack' ? { y: -20, scale: 1.1 } : { y: 0, scale: 1 }} transition={{ duration: 0.2, delay: idx * 0.1, yoyo: Infinity }} className={`w-20 ${getRarityColor(card.rarity)} border-4 border-b-8 rounded-2xl p-1.5 flex flex-col items-center shadow-lg relative ${isReady && isCombat ? 'cursor-pointer ring-4 ring-yellow-400 animate-pulse scale-110' : ''}`}>
                                    ...
                                    <div className="absolute -left-3 top-0 bottom-0 w-2 bg-black/50 rounded-full overflow-hidden border border-black/20"><div className="w-full bg-yellow-400 bottom-0 absolute transition-all" style={{ height: `${cardCharges[idx]}%` }} /></div>
                                    <div className="absolute -top-2 -right-2 bg-[#022c22] border border-[#0f766e] rounded-full w-6 h-6 flex items-center justify-center shadow-sm z-20">{getElementIcon(card.element)}</div>
                        ...
                        <button onClick={simulateInvasion} disabled={combatState.status !== 'idle' || activeDeck.length === 0} className="absolute bottom-6 right-6 z-50 bg-purple-600 text-white border-4 border-purple-800 border-b-8 active:border-b-4 active:translate-y-1 font-black text-xs uppercase px-5 py-4 rounded-[2rem] shadow-[0_0_15px_rgba(168,85,247,0.5)] flex items-center gap-2 transition-all disabled:opacity-50 disabled:grayscale">
                            <AssetSkull className="w-5 h-5"/> {combatState.status !== 'idle' ? 'Lutando...' : activeDeck.length === 0 ? 'Sem Runas' : wave % 5 === 0 ? 'Enfrentar Lorde' : 'Purificar'}
                        </button>
                    </div>
            ...
            {activeTab === 'laboratorio' && (
                <motion.div key="laboratorio" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col gap-6">
                    <div className="bg-[#042f2e] border-4 border-[#022c22] rounded-[2.5rem] p-8 shadow-xl flex flex-col items-center">
                        <div className="bg-teal-800 border-4 border-teal-600 p-4 rounded-[2rem] shadow-md mb-6"><AssetRuneStone className="w-12 h-12 text-teal-200" /></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                            <div className="bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-5 flex flex-col items-center text-center shadow-lg"><h3 className="text-sm font-black text-teal-100 mb-1 uppercase">Visão do Além</h3><p className="text-[11px] text-teal-600 font-black uppercase mb-4 bg-[#042f2e] px-3 py-1 rounded-full border-2 border-[#022c22]">Nível {research.radarCooldown}</p><p className="text-xs text-purple-300 font-bold mb-6">-5s no recarregamento da Selva por nível.</p><button onClick={() => researchTech('radar')} className="mt-auto w-full py-4 bg-teal-600 border-4 border-teal-800 border-b-8 active:border-b-4 active:translate-y-1 text-white rounded-2xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2">Abençoar <span className="bg-teal-700 px-2 py-1 rounded-xl border-2 border-teal-500 flex items-center gap-1 shadow-sm text-sm"><AssetReliquia className="w-4 h-4" /> {radarCost}</span></button></div>
                            <div className="bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-5 flex flex-col items-center text-center shadow-lg"><h3 className="text-sm font-black text-teal-100 mb-1 uppercase">Raízes Férteis</h3><p className="text-[11px] text-teal-600 font-black uppercase mb-4 bg-[#042f2e] px-3 py-1 rounded-full border-2 border-[#022c22]">Nível {research.minerBonus}</p><p className="text-xs text-teal-300 font-bold mb-6">+1 Recurso por Golem a cada ciclo.</p><button onClick={() => researchTech('miner')} className="mt-auto w-full py-4 bg-teal-600 border-4 border-teal-800 border-b-8 active:border-b-4 active:translate-y-1 text-white rounded-2xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2">Abençoar <span className="bg-teal-700 px-2 py-1 rounded-xl border-2 border-teal-500 flex items-center gap-1 shadow-sm text-sm"><AssetReliquia className="w-4 h-4" /> {minerCost}</span></button></div>
                            <div className="bg-[#022c22] border-4 border-[#064e3b] rounded-[2rem] p-5 flex flex-col items-center text-center shadow-lg"><h3 className="text-sm font-black text-teal-100 mb-1 uppercase">Casca de Aether</h3><p className="text-[11px] text-teal-600 font-black uppercase mb-4 bg-[#042f2e] px-3 py-1 rounded-full border-2 border-[#022c22]">Nível {research.coreBonus}</p><p className="text-xs text-emerald-300 font-bold mb-6">+100 HP Máximo para o seu Núcleo.</p><button onClick={() => researchTech('core')} className="mt-auto w-full py-4 bg-teal-600 border-4 border-teal-800 border-b-8 active:border-b-4 active:translate-y-1 text-white rounded-2xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2">Abençoar <span className="bg-teal-700 px-2 py-1 rounded-xl border-2 border-teal-500 flex items-center gap-1 shadow-sm text-sm"><AssetReliquia className="w-4 h-4" /> {coreBonusCost}</span></button></div>
                        </div>
                    </div>
                </motion.div>
            )}
